...
let ``healing does not exceed limit``() =
    let specs = [{ Pattern = "test_building"; SubBlocks = [| 0; 1 |]; Production = 25.0f<E/H>; Storage = 0.0f<E>; IsAirfield = false; Durability = 25000 }]
    let groups = [{ Model = "test_building"; Script = ""; Pos = { Pos = Vector2.Zero; Rotation = 0.0f; Altitude = 0.0f } }]
    let healLimit = 30.0f<E>
    let healths, energy = computeHealing(specs, [[| 0.0f; 1.0f; 0.5f |]], groups, 100.0f<E>, healLimit)
