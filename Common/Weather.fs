...
module Campaign.Common.Weather
...
let setOptions (random : System.Random) (weather : WeatherState) (t : System.DateTime) (options : T.Options) =
    ...
    let windLayers =
        ...
        |> Seq.map(fun trip ->
            ...
            T.Options.WindLayers.WindLayers_ValueType.Create(alt, T.Integer.N dir, T.Integer.N speed)
        )
    options
        .SetDate(T.Date.FromDate(t.Day, t.Month, t.Year))
        .SetTime(T.Options.Time.Create(T.Integer.N t.Hour, T.Integer.N t.Minute, T.Integer.N t.Second))
        .SetCloudLevel(T.Integer.N (int(floor(weather.CloudHeight))))
        .SetCloudHeight(T.Integer.N (int(floor(weather.CloudThickness))))
        .SetPrecType(T.Integer.N precType)
        .SetPrecLevel(T.Integer.N precLevel)
        .SetCloudConfig(T.String.N cloudConfig)
        .SetTurbulence(T.Float.N weather.Turbulence)
        .SetTemperature(T.Integer.N (int(floor(weather.Temperature))))
        .SetWindLayers(T.Options.WindLayers.FromList (windLayers))
        .SetSeaState(T.Integer.N (int (6.0 * weather.Wind.Speed / 40.0) |> min 6))